...
(function () {
  ...
  var EX = {
    ...
    "Fentes avant alternées": {
      sides: "alternate", camera: { yaw: 1.05 }, start: "up",
      poses: {
        up: STAND(0.1, 0),
    ...
    "Pont de hanche": {
      camera: { yaw: 1.2, pitch: 0.38, dist: 3.4, ty: 0.25, tz: 0 }, start: "down",
    ...
    "Élévation des talons": {
      camera: { yaw: 1.3 }, props: ["wall"], start: "down",
      poses: {
    ...
    "Step-up sur marche": {
      sides: "alternate", camera: { yaw: 1.0 }, props: ["step"], start: "floor",
      poses: {
  ...
  /* ── Lecture ── */
  var def = null, current = null, tween = null, raf = 0, host = null;
  ...
  function frame(now) {
    ...
    applyPose(current);
    var c = def.camera || {}, v = R.view;
    v.yaw += (v.yawGoal - v.yaw) * 0.12;
    var d = c.dist || 4.1, pitch = c.pitch || 0.15, ty = c.ty != null ? c.ty : 0.88, tz = c.tz || 0;
    R.camera.position.set(d * Math.sin(v.yaw) * Math.cos(pitch), ty + d * Math.sin(pitch), tz + d * Math.cos(v.yaw) * Math.cos(pitch));
